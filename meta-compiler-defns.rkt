...
;Problem 2 compile-application that allows compiling more than 1 arguments
...
;Problem 2 compile-lambda that allows compiling more than 1 arguments
(define (compile-lambda binders exp env-names) 
  (let ((body-code (compile exp (cons binders env-names))))
    (lambda (env-values cont) 
      (cont (lambda (args k)
              (body-code (cons args env-values) k))))))
  ...
  (let ((body-code (compile exp env-names)))
    (lambda (env-values cont)
        (body-code env-values
              (lambda (m) (m (list (lambda (x k) (apply cont x))) cont)))))) 
...
(define (call/cc-body exp) (cadr exp))
...
(define (lookup-variable v env-names)
  (define (find i j env)
    (cond ((null? (car env)) (find (1+ i) 0 (cdr env)))
          ((eq? (caar env) v) (list i j))
          (else (find i (1+ j) (cons (cdar env) (cdr env))))))
  (find 0 0 env-names))
